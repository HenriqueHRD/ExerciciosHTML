	...
	var self = {},
		...
		limparElementos = function() {
			var casas = elemento.querySelectorAll('.casa'), 
				casas = Array.prototype.slice.call(casas);

			casas.forEach(function(el) {
				el.innerText = '';
			});
		},
		...
		verificaVencedor = function() {
			...
			if (ganhouDiagonal){
				alert("Jogador " + (!status ? "O": "X")+ " ganhou na diagonal");
			}else if (ganhouLinha){
				alert("Jogador " + (!status ? "O": "X") + " ganhou na linha");
			}else if (ganhouColuna){
				alert("Jogador " + (!status ? "O": "X") + " ganhou na coluna");
